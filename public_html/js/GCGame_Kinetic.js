...
function initCards() {
    ...
    for (i; i < maxCardCount; i++) {
        var baseX = 10 + i * (cardWidth + 20);
        var baseY = 200;
        type = Math.floor((Math.random() * 4));
        var card = new Card(type, cardLayer, 0, baseX, baseY);
        myCards.push(card);
        intMouseInOutHandler(card);
    }

    for (i = 0; i < maxCardCount; i++) {
        baseX = 10 + (i + 1) * (cardWidth + 20);
        baseY = 10;
        type = Math.floor((Math.random() * 4));
...
function intMouseInOutHandler(card) {
    ...
    card.bg.on('click', function() {
        for (var i = 0; i < myCards.length; i++) {
            if (myCards[i].pattern === card.pattern)
                break;
        }
        toggleTapped(i);
    });
    card.pattern.on('click', function() {
        for (var i = 0; i < myCards.length; i++) {
            if (myCards[i].pattern === card.pattern)
                break;
        }
...
function toggleTapped(i) {
    var obj = {bg: myCards[i].bg, pattern: myCards[i].pattern, factor: 0};
    if (myCards[i].tapped) {
        ...
        myCards[i].tapped = true;
    }
    tappedCards.length = 0;
    for (i = 0; i < myCards.length; i++) {
        if (myCards[i].tapped) {
            tappedCards.push(myCards[i]);
        }
    }
}
...
function removeCard(i) {
    ...
    for (i; i < myCards.length; i++) {
        myCards[i].bg.setX(myCards[i].bg.getX() - cardWidth - 20);
        myCards[i].pattern.setX(myCards[i].pattern.getX() - cardWidth - 20);
    }
}
...
function initStats() {
    if (player1.health <= 0) {
        ...
        var tempHtml = "<p>Health points: " + player1.health.toString() + "</p>"
                + "<p>Number of cards: " + player1.deck.length + "</p>";
        $('#myStats').html(tempHtml);
        tempHtml = "Opponent's health points:" + player2.health.toString() + "</p>";
